...
| `LIVECITY_IGNOREBLOCKER` | `Engine.IgnoreJunctionBlockerSeconds` — bounded patience for every physical-occupancy hold, SUMO's own `--ignore-junction-blocker` (seconds; a foe standing at least this long constrains nobody; `-1` = never ignore). **Behavioural.** Unset ⇒ **60 when `LIVECITY_F3OCCUPANCY=1`, engine default (−1) otherwise** — the Entry-37 anti-wedge default: a 5-vehicle jy7→admission→leaderFollow ring wedged one junction and cascaded citywide (arrivals halved) until this edge was cut | 60 (gate on) / −1 | behavioural |
| `LIVECITY_TRACEVEH` | `Engine.DiagTraceVehicleId` — a vehicle id (e.g. `__veh1967`), not a boolean; the same per-vehicle constraint tracing the Sim.Run drivers expose as `SUMOSHARP_TRACEVEH` (`[merge]`/`[bay]`/`[keepclear]`/… stderr lines), plumbed into the live-city host for wedge-episode traces (Entry 38). **Diagnostic only — changes no trajectory** | no trace | `null` |
| `LIVECITY_REROUTE` | LiveCity master switch for the engine's `device.rerouting` port (P1E, golden-pinned; docs/LIVECITY-REROUTING-DESIGN.md). **DEFAULT ON since Entry 47** (owner decision: period 60 s, probability 1.0 — all drivers navigate): `0` is the KILL SWITCH (no splice), `1` force-enables a host that constructed with an explicit period 0. **Behavioural** (cars periodically re-shortest-path on live smoothed edge travel times). Any A/B or SUMO-comparison run must set it explicitly in both arms | **on** (60 s / 1.0) | behavioural |
| `LIVECITY_REROUTE_PERIOD` | Seconds between per-vehicle periodic reroute checks (`device.rerouting.period`); overrides both the `LIVECITY_REROUTE=1` default (60) and any host-supplied `LiveCityConfig.ReroutePeriodSeconds`. **Behavioural.** Also ENABLES the device by itself when > 0 | 60 (when enabled) | behavioural |
| `LIVECITY_REROUTE_PROB` | Fraction of cars equipped with the rerouting device (`device.rerouting.probability`, per-entity seeded equip draw); overrides any host-supplied `LiveCityConfig.RerouteProbability`. **Behavioural** (only meaningful while the device is enabled) | 1.0 | behavioural |
| `LIVECITY_URGENTFOLLOW` | `Engine.UrgentStrategicLeaderFollow` — the Entry-31 urgent-strategic-follow arm (binder 18), mirrored into the live-city hosts for A/B after the 3D session measured it as the dominant saturated-Geneva mid-lane stall class (14 vs 1 at matched windows). Same engine flag Sim.Run/SumoShim expose as `SUMOSHARP_URGENTFOLLOW`. **Behavioural.** `0` disables the arm | engine default (on) | behavioural |
| `LIVECITY_PARTIALVEH` | `Engine.PartialOccupancyGate` — the **myPartialVehicles port** (docs/PARTIAL-OCCUPANCY-DESIGN.md): a vehicle whose body spans a lane boundary stays visible on the lane(s) its tail covers, to the same-lane leader fold and the cross-junction rearmost (phase 1). Cures the Entry-53 stopped-lookahead ratchet (the "drive through junction blockers" overlap class). **Engine default ON** (SUMO-faithful); `0` = kill switch — set explicitly in both arms of any A/B. `Sim.Run`/drop-in name: `SUMOSHARP_PARTIALVEH` | `true` | behavioural |
| `LIVECITY_RINGBREAK` | `Engine.RingBreakGate` — DEADLOCK-RING **D2**, the gated ring break (docs/DEADLOCK-RING-DESIGN.md §2): a blocker-graph ring confirmed ≥ 20 s elects ONE member (entry-order chain; releasable stop-edges only — keepClear/admission) and skips that member's stop-edge toward its ring target so the follow-form arms creep it through gaps, never bodies; wedged breakers escalate, exhausted rings report honest `stuckSteps` (host prints `LIVECITY-RINGBREAK` under the witness cadence). **Behavioural.** **Engine default ON since Entry 56** (owner decision; partials cured its Entry-52 landing blocker) — `0` is the kill switch. Deliberately NOT in the forced junction-gate bundle — set explicitly in both arms of any A/B, like `LIVECITY_F3OCCUPANCY` | `true` | behavioural |
...
| `CITY3D_PEDAVOIDCARS` | `LiveCitySim.PedAvoidCarsInZone` in the City3D host (`LiveCitySource` ctor): the camera-zone opt-in for PED-AVOID-CARS above. Read with `!= "0"`, so **on unless explicitly `0`** in the 3D host | `true` (3D host) | behavioural |
...
| `LIVECITY_PEDORCALOG` | PEDORCA diagnostic (the ORCA-runaway hunt): print-only census of the high-power ped population — `[pedorca] CENSUS` (high count, runaways = peds >25 m off their OWN steering path, RecoverRoute arm totals ok/spliced/beeline) + `[pedorca] RUNAWAY` exemplars. Changes no behaviour | off | diagnostic |
| `LIVECITY_PEDAVOIDCARS` | PED-AVOID-CARS (Entry 69): high-power ORCA peds avoid NEAR-STOPPED cars (< 1.5 m/s) inside the LC-realism zone — each such car becomes a chain of footprint discs fed to `OrcaCrowd.SetExternalObstacles` via the (previously always-empty) `externalEntities` seam. three-state: `1` force-on, `0` force-off, unset ⇒ `LiveCitySim.PedAvoidCarsInZone` (sim default **OFF** — measured hour-horizon harm on the saturated demo box: 30/104 long stalls vs a 0 guard; the 3D host opts in via `CITY3D_PEDAVOIDCARS`). Scope: cars on INTERNAL (junction) lanes only. **Behavioural** for ped poses in-zone (and, through car-brakes-for-ped, cars near them) | off (sim) / on (3D host) | behavioural |
| `LIVECITY_LCZONE_RADIUS` | headless stand-in for the camera-driven LC-realism zone: widens the zone (ORCA promote pocket, car→ped yield, and — with `HighRealismFollowsZone` — the pass-through mask) to this radius (m) at the static pocket centre, via `SetLcRealismZone` at the first step. **Behavioural** where it covers traffic (promotions change ped poses) | unset = ctor 70 m pocket | behavioural |
...
| `SUMOSHARP_PARTIALVEH` | `Engine.PartialOccupancyGate` — the **myPartialVehicles port** (docs/PARTIAL-OCCUPANCY-DESIGN.md): boundary-spanning bodies stay visible on the lane(s) their tail covers (same-lane leader fold + cross-junction rearmost, phase 1). **Behavioural.** Default ON (SUMO-faithful; cures the Entry-53 stopped-lookahead ratchet). Read by `Sim.Run` AND the `sumosharp` drop-in, both in the safe `EnvGate` form. LiveCity name: `LIVECITY_PARTIALVEH` | **engine default** | `true` |
...
| `SUMOSHARP_PHYSOCCUPANCY` | `Engine.JunctionPhysicalOccupancyGate` — the F3 physical-occupancy arm: `FoeWith`-widened `AdaptToJunctionLeader` (a foe PHYSICALLY on a geometrically-conflicting internal lane constrains ego via the `isLeader() \|\| inTheWay()` disjunction, regardless of right-of-way). **Behavioural.** Default OFF; the F3-era "counterproductive alone" verdict predates its sibling gates shipping default-ON and the Entry-30 determinism fix — re-measurement is JUNCTION-FOE-LANE F2.1 (journal Entry 35). Read by `Sim.Run` AND the `sumosharp` drop-in, both in the safe `EnvGate` form | **engine default** | `false` |
| `SUMOSHARP_IGNOREBLOCKER` | `Engine.IgnoreJunctionBlockerSeconds` — seconds; parses a double, `-1` = never ignore (SUMO parity). **Behavioural** measurement plumbing for the Entry-37 patience escape; unset ⇒ engine/config default so every existing A/B is untouched. Read by `Sim.Run` only (the `sumosharp` drop-in drives the same property via `--ignore-junction-blocker`/cfg) | **engine default** | `-1` |
