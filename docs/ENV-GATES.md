...
| `SUMOSHARP_URGENTFOLLOW` | `Engine.UrgentStrategicLeaderFollow` — the `informLeader` urgent-strategic leader-follow coupling (brake to slot in behind the target-lane leader instead of waiting for a gap two equal-speed vehicles cannot open). **Behavioural.** Default OFF pending `docs/URGENT-STRATEGIC-FOLLOW-DESIGN.md` sign-off | **engine default** | `false` |
