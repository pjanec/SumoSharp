...
| `LIVECITY_RINGBREAK` | `Engine.RingBreakGate` — DEADLOCK-RING **D2**, the gated ring break (docs/DEADLOCK-RING-DESIGN.md §2): a blocker-graph ring confirmed ≥ 20 s elects ONE member (entry-order chain; releasable stop-edges only — keepClear/admission) and skips that member's stop-edge toward its ring target so the follow-form arms creep it through gaps, never bodies; wedged breakers escalate, exhausted rings report honest `stuckSteps` (host prints `LIVECITY-RINGBREAK` under the witness cadence). **Behavioural.** Deliberately NOT in the forced junction-gate bundle — set explicitly in both arms of any A/B, like `LIVECITY_F3OCCUPANCY` | `false` | behavioural |
