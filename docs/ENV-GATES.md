...
| `LIVECITY_REROUTE` | LiveCity master switch for the engine's `device.rerouting` port (P1E, golden-pinned; docs/LIVECITY-REROUTING-DESIGN.md): `1` splices `device.rerouting.period/probability` into the LiveCitySim engine config (period 60 s, probability 1.0 unless the two vars below override). **Behavioural** (cars periodically re-shortest-path on live smoothed edge travel times). Unset/other + cfg default ⇒ NO splice, engine config byte-identical to the pre-rerouting build | off | behavioural |
