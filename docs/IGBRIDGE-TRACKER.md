...
- [~] **T2.1 (experimental)** Anticipatory turn-in (owner: "a real car would go a bit farther before starting the turn"): turn-rate-limited steered front → wide line on sharp corners (v108 peak curvature 537→120 °/s, reconverges). Works visually but steering along the faceted lane heading raises fleet median yaw-accel reversals 0→1 (max 13). Shipped OFF by default (`TurnInRateDegPerSec=0`; `IGBRIDGE_TURNIN=65` to demo); default = exact v2. Needs kink refinement before default-on. See DECISIONS §5.10.
