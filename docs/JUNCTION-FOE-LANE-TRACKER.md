...
- [~] F1.1 — the stopXmove class decomposed by trace into THREE mechanisms (journal Entry 39 BEFORE/MID/AFTER-A): (A) actual-lane link resolution **DONE at DEFAULTS** (succLinkSec parity; L2 gate-ON stopXmove 19→9, defaults 17→13, landings 10→6; non-foes ingest rows + pin test also landed); (B) late-stop race → corridor-follow design, OPEN (next, ~6/9 remaining); (C) straddling-tail corner-cut → named residual (~2/9). New L2 hashes: default `5ac89389…`, gate-ON `fd636381…`; bench hash unchanged
