# Spectacle ↔ proprietary IG binding — DESIGN (the HOW)

How to feed a proprietary 2-sample IG from SumoSharp with all core motion artifacts smoothed, and build a
PoC that proves it without the real IG. Requirements (the WHAT) are in `SPECTACLE-IG-BINDING-REQUIREMENTS.md`;
...
Therefore **all smoothing must be baked into the sample stream Spectacle emits** — the emitted samples must
lie on a smooth curve, dense enough that the IG's linear/slerp between any two consecutive samples already
looks right. Spectacle is a **reconstruction + resampling stage**, not a passthrough. In particular a
...
Spectacle adds *transport + resampling + 2D→3D + lifecycle*, never new curve/heading math.

## 2. Architecture — three decoupled stages
```
 [1] SumoSharp core @ fixed 10 Hz  ──samples──▶  [2] shared reconstruction  ──pose(t)──▶  [3] Spectacle emit
...
```
- **[1] Fixed-rate core.** Own 100 ms sim clock; never driven by Spectacle's variable wall tick. Each step
...
- **[3] Spectacle stage.** Advances a **sim clock** (§4), and at the IG send cadence queries [2] at a single
...
The reconstruction in [2] produces a *continuous* pose(t); Spectacle samples that continuous curve at the IG
...
- Decouple **sim time** from wall time. Spectacle plays at `simClock = f(wall)` (1× real-time by default),
...
## 5. 2D → 3D (Spectacle-added, not in the 2D viewers)
...
- **Junction turns** → `DrClock.Resolve`'s **arc-window walk** across lane-crossing brackets + `ChordHeading`
  (DR doc §6.1/§6.2). Already solved for the 2D viewer; Spectacle inherits it.
...
- **Instant lane change** → the straddle Cartesian-lerp gives a lateral move, but over the *bracket* interval
  ...
  viewer benefits too.
- **Overlaps** → NOT smoothed here (a sim defect; separate spec). Spectacle just renders faithfully.
...
## 9. The PoC — `Sim.Spectacle` (proposed)
...
- Nothing here touches `Sim.Core`'s parity path — Spectacle/`Sim.Spectacle` are consumers. The offline
...
- **Stage 0 — survey & wire.** Read `SUMOSHARP-VIEWER-DR-SMOOTHING.md` (esp §8) + `Sim.Viewer.Motion`. Stand up
  `Sim.Spectacle`: fixed-10 Hz core, per-entity ring buffers, sim clock. Confirm `Sim.Viewer.Motion` is
...
2. **Emit cadence & interp delay:** does the IG fix the delay, or may Spectacle choose the cadence? (Sets the
...
5. **Terrain source of truth:** is the City3D procedural terrain the authority for `z`/slope the IG should
   match, or does the IG own its own terrain (then Spectacle sends only planar pose + yaw)?
