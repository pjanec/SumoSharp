# TASKS-DONE.md — Archive (completed work + full roadmap/task detail)

**This is the ARCHIVE.** It holds the full text of completed tasks AND the detailed characterizations
of the parity/realism roadmap, kept verbatim so nothing is lost and so the short open-item queue in
**`TASKS-TODO.md`** can point here for detail. The live, short work queue is `TASKS-TODO.md`; consult
that first. Items below marked **DONE / ✅ / ACCEPT / landed** are finished; the characterized-but-not-
yet-briefed roadmap items are tracked (as one-liners) in `TASKS-TODO.md`.
