...
| F3 junction overlap + F4b | `claude/f3-junction-overlap-handoff-okf5nu` | **STARTED** | pre-existing junction car–car overlap (into-occupied / keep-clear) + zero-overlap invariant. Edits `Engine.cs` **junction** methods (`JunctionYieldConstraint` etc.). Brief: **`docs/F3-JUNCTION-OVERLAP-HANDOFF.md`** |
