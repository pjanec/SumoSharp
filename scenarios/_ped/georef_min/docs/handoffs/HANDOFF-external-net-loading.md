...
## Change 3 - live pedestrian-count setter

**Problem.** Ped count (`PedPopulationCap`, `PedSpawnRatePerSecond`) is copied once into an `init`-only
`PedDemandConfig` at `LiveCitySim` ctor (`LiveCitySim.cs:272-278`; `PedDemandConfig` props are `init`-only,
`src/Sim.Pedestrians/Demand/PedDemand.cs:718,721`). So a free-style ped-count slider cannot take effect
without a full sim rebuild. Cars are already live (Step reads `_cfg.CarTargetConcurrent` each tick); peds are
not.

**Change (additive).** Make ped demand density live: e.g. `PedDemand.SetPopulationCap(int)` /
`SetSpawnRatePerSecond(double)` (mutate a private field the spawn loop reads each step), plus a passthrough
`LiveCitySim.SetPedDensity(int cap, double ratePerSec)` (and/or expose the `PedDemand`). Then a slider that
calls it takes effect next tick, no rebuild.

**Who benefits.** BIG Spectacle (its ped sliders currently apply on Restart for exactly this reason) AND the
Godot City3D viewer (`HANDOFF-godot-city3d-arbitrary-net.md`, task T3). Once this lands, BIG wires its ped
sliders live (removing the "applies on Restart" note) and City3D gets a live ped slider.

**Validate:** call the setter mid-run; assert the live ped count converges toward the new cap within a few
seconds, with no sim rebuild.
...
3. Live ped-density setter (Change 3) lands on `main`; ped count changes without a sim rebuild.
4. All validated headless in isolation (harnesses described above). BIG then wires the Spectacle loader,
   georef mapping, ped Z, and live ped sliders; the Godot session picks up C1/C2/C3 for its own tasks.
