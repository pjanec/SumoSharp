...
def generate_net(out_dir, a):
    ...
    if a.tls_guess:
        cmd += ["--tls.guess", "true", "--tls.guess.threshold", str(a.tls_thresh),
                "--tls.discard-simple", "false"]
    else:
        cmd += ["--tls.discard-simple", "true"]
    run(cmd)
    return net_path
...
def main():
    ...
    # Defaults are the WINNING v6 config (witness residual): vanilla teleports=0,
    # SumoSharp teleports=42 (jam=13, yield=29). See README.md.
    p.add_argument("--out", default=HERE)
    p.add_argument("--iters", type=int, default=140)
    p.add_argument("--mind", type=float, default=22.0)
    p.add_argument("--maxd", type=float, default=90.0)
    ...
    # TL guessing is ON by default: a handful of busy junctions become traffic
    # lights -- the dominant lever that makes SumoSharp over-teleport vs vanilla
    # (the real-box Y1 pattern). --no-tls-guess disables it (net inverts to van>ss).
    p.add_argument("--no-tls-guess", dest="tls_guess", action="store_false")
    p.set_defaults(tls_guess=True)
    p.add_argument("--tls-thresh", dest="tls_thresh", type=float, default=110.0)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--through", type=int, default=280)
    p.add_argument("--park-stay", dest="park_stay", type=int, default=35)
    p.add_argument("--depart-parked", dest="depart_parked", type=int, default=10)
    p.add_argument("--sinkcap", type=int, default=6)
    p.add_argument("--sinkbay", type=float, default=16.0)
    p.add_argument("--period", type=float, default=1.7)
