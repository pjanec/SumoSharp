...
// Rung A2's real parity test: a fast follower overtakes a slow leader (maxSpeed=5) on a 2-lane
// edge -- the speed-gain (overtaking) lane-change decision (DESIGN.md Seam 4, ported from
// MSLCM_LC2013::_wantsChange's LEFT-change block) fires the LEFT change at t11->t12, then rung
// 8b's existing keep-right accumulator (unchanged) returns the follower to the right lane at
// t19->t20. This is the first test to exercise a lane-change decision driven by a LEADER's
// post-move gap rather than an empty road.
public class RungA2ParityTests
{
    ...
    private static string BuildFailureMessage(ComparisonResult result)
    {
        ...
        {
            $"Rung-A2 parity FAILED. FirstDivergenceStep={result.FirstDivergenceStep?.ToString() ?? "none"}",
        };
